...
@Component({
  selector: 'app-union-hommes-missionnaires',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, PaginationComponent],
  templateUrl: './union-hommes-missionnaires.component.html',
  styleUrls: ['./union-hommes-missionnaires.component.css']
})
export class UnionHommesMissionnairesComponent implements OnInit {
  membres: Membres[] = [];
  ...
  async loadDepartementInfo(): Promise<void> {
    try {
      ...
      this.departement = departements.find(d => d.libelle === "Union des Hommes Missionnaires") || null;
      if (this.departement) {
  ...
  async loadPagination(): Promise<void> {
    if (!this.departement || this.isLoading) return;
  ...
  async filterMembres(): Promise<void> {
    await this.search();
  }
